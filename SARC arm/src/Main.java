...
public class Main {
	...
	/**
	 * Starts the toolPath class, which creates a set of steps that the motor should move to in order to produce
	...
	public void doSendFile() {
		...
		/**
		 * try { tool_path = new ToolPath(); tool_path.convert_drawing_to_angles(drawing, arm, "does nothing");
		 * tool_path.convert_angles_to_pwm(arm); tool_path.save_pwm_file();
		 * 
		 * Runtime.getRuntime().exec("scp PWMToSend pi@10.140.141.22:/home/pi/arm");// sends the text file "PWMToSend"
		 * // to the pi. I think can only // be executed on lab computers // MIGHT need to add passwords } catch
		 * (IOException e) { UI.println("Something went wrong " + e); }
		 */
	}
	...
	public void doMouse(String action, double x, double y) {
		...
		if (((state == 2) || (state == 3)) && action.equals("moved")) {
			// draw arm and path
			arm.inverseKinematic(x, y);
			// toSendText.add(String.valueOf(arm.get_theta1())+ ","+String.valueOf(arm.get_theta2())+",0");//0 is
			// up/down pen
			arm.draw();
		...
		if ((state == 2) && (action.equals("clicked"))) {
			...
			arm.inverseKinematic(x, y);
			toSendText.add(this.convertToPWM(arm.get_theta1(), true) + "," + this.convertToPWM(arm.get_theta2(), false)
					+ ",2000");// 2000 is up/down pen
			arm.draw();
			drawing.draw();
			drawing.print_path();
		}
	...
	public String convertToPWM(double angle, boolean motor1) {
		...
		// angle should be converted into PWM String value 0 for 0 radian angle, each degree extra corresponds to about
		// 10PWM, increase
		angle = -Math.toDegrees(angle);
		double variable = motor1 ? 170 : 770; // 170 = motor 1, 770 = motor 2
